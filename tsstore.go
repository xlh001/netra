...
var tsTableDDL = map[string]string{
	tblFlow: `CREATE TABLE flow_samples (ts BIGINT, src_ip BIGINT, dst_ip BIGINT, src_port BIGINT, dst_port BIGINT, proto BIGINT, packets BIGINT, bytes BIGINT, domain VARCHAR, dpi_service VARCHAR, svc_port BIGINT)`,
	tblIP:   `CREATE TABLE ip_samples (ts BIGINT, ip BIGINT, packets BIGINT, bytes BIGINT)`,
	tblPort: `CREATE TABLE port_samples (ts BIGINT, proto BIGINT, port BIGINT, packets BIGINT, bytes BIGINT, dpi_service VARCHAR)`,
}
...
func flowTotalCacheKey(cutoff, until int64, ipFilter *uint32, q string, dpiOnly bool) string {
	ipKey := "nil"
	if ipFilter != nil {
		ipKey = strconv.FormatUint(uint64(*ipFilter), 10)
	}
	return fmt.Sprintf("flow|%d|%d|%s|%s|%t", cutoff/totalCacheBucketSeconds, until/totalCacheBucketSeconds, ipKey, q, dpiOnly)
}

func ipOctetExpr(col string) string {
	return fmt.Sprintf(
		`CONCAT(CAST(%s & 255 AS VARCHAR), '.', CAST((%s >> 8) & 255 AS VARCHAR), '.', CAST((%s >> 16) & 255 AS VARCHAR), '.', CAST((%s >> 24) & 255 AS VARCHAR))`,
		col, col, col, col,
	)
}
...
func computeFlowFileTopK(path string, start, end int64) (flowFileCacheEntry, error) {
	...
	q := fmt.Sprintf(`SELECT src_ip, src_port, dst_ip, dst_port, proto, SUM(packets), SUM(bytes), MAX(domain), MAX(dpi_service), MAX(svc_port)
	...
	for rows.Next() {
		...
		var svcPort sql.NullInt64
		if err := rows.Scan(&srcIP, &srcPort, &dstIP, &dstPort, &proto, &packets, &bytes, &domain, &dpiService, &svcPort); err != nil {
			rows.Close()
		...
		out = append(out, flowAggRow{
			...
			Packets: uint64(packets), Bytes: uint64(bytes), Domain: domain.String, DPIService: dpiService.String, SvcPort: uint16(svcPort.Int64),
		})
	}
	rowsErr := rows.Err()
	rows.Close()
...
func (t *tsStore) writeTick(ts int64, flows []flowSample, ips map[uint32]xdpflowFlowStats, ports map[portKey]xdpflowFlowStats, portDPI map[portKey]string) error {
	...
	for _, f := range flows {
		...
		var svcPort any
		if f.svcPort != 0 {
			svcPort = int64(ntohs(f.svcPort))
		}
		if err := flowApp.AppendRow(ts, int64(f.key.Saddr), int64(f.key.Daddr), int64(ntohs(f.key.Sport)), int64(ntohs(f.key.Dport)), int64(f.key.Proto), int64(f.packets), int64(f.bytes), domain, dpiService, svcPort); err != nil {
	...
	for pk, stat := range ports {
		var dpiService any
		if svc := portDPI[pk]; svc != "" {
			dpiService = svc
		}
		if err := portApp.AppendRow(ts, int64(pk.proto), int64(ntohs(pk.port)), int64(stat.Packets), int64(stat.Bytes), dpiService); err != nil {
...
func flowSealedSource(files []string) string {
	return fmt.Sprintf(`(SELECT * FROM read_parquet(%s, union_by_name=true) UNION ALL BY NAME SELECT NULL::BIGINT AS ts, NULL::BIGINT AS src_ip, NULL::BIGINT AS dst_ip, NULL::BIGINT AS src_port, NULL::BIGINT AS dst_port, NULL::BIGINT AS proto, NULL::BIGINT AS packets, NULL::BIGINT AS bytes, NULL::VARCHAR AS domain, NULL::VARCHAR AS dpi_service, NULL::BIGINT AS svc_port WHERE FALSE)`, parquetFileList(files))
}
...
func querySealedAgg[T any](t *tsStore, table string, buildSQL func(source string) string, cutoff, until int64, scan func(*sql.Rows) ([]T, error)) ([]T, error) {
	return querySealedAggSource(t, table, func(files []string) string {
		return "read_parquet(" + parquetFileList(files) + ")"
	}, buildSQL, cutoff, until, scan)
}

func querySealedAggSource[T any](t *tsStore, table string, source func(files []string) string, buildSQL func(source string) string, cutoff, until int64, scan func(*sql.Rows) ([]T, error)) ([]T, error) {
	...
	rows, err := scratch.Query(buildSQL(source(files)), cutoff, until)
	if err != nil {
	...
	add := func(rows []flowAggRow) {
		for _, r := range rows {
			if existing, ok := acc[r.Key]; ok {
				...
				if existing.SvcPort == 0 && r.SvcPort != 0 {
					existing.SvcPort = r.SvcPort
				}
			} else {
				cp := r
				acc[r.Key] = &cp
...
func mergePortAgg(a, b []portAggRow) []portAggRow {
	acc := map[tsPortKey]*portAggRow{}
	add := func(rows []portAggRow) {
		for _, r := range rows {
			if existing, ok := acc[r.Key]; ok {
				existing.Packets += r.Packets
				existing.Bytes += r.Bytes
...
func portSealedSource(files []string) string {
	return fmt.Sprintf(`(SELECT * FROM read_parquet(%s, union_by_name=true) UNION ALL BY NAME SELECT NULL::BIGINT AS ts, NULL::BIGINT AS proto, NULL::BIGINT AS port, NULL::BIGINT AS packets, NULL::BIGINT AS bytes, NULL::VARCHAR AS dpi_service WHERE FALSE)`, parquetFileList(files))
}
...
func (t *tsStore) queryIPTimeline(cutoff, until int64, ip uint32) ([]aggRow[int64], error) {
	where := fmt.Sprintf("ts >= ? AND ts <= ? AND (src_ip = %d OR dst_ip = %d)", ip, ip)
	buildSQL := func(source string) string {
		return fmt.Sprintf(`SELECT ts, SUM(packets), SUM(bytes) FROM %s WHERE %s GROUP BY ts`, source, where)
	}
	scan := func(rows *sql.Rows) ([]aggRow[int64], error) {
		defer rows.Close()
		var out []aggRow[int64]
		for rows.Next() {
			var ts, packets, bytes int64
			if err := rows.Scan(&ts, &packets, &bytes); err != nil {
				return nil, err
			}
			out = append(out, aggRow[int64]{Key: ts, Packets: uint64(packets), Bytes: uint64(bytes)})
		}
		return out, rows.Err()
	}

	sealed, err := querySealedAggSource(t, tblFlow, flowSealedSource, buildSQL, cutoff, until, scan)
	if err != nil {
		return nil, fmt.Errorf("query sealed flow_samples timeline: %w", err)
	}

	t.mu.RLock()
	hotRows, err := t.hotDB.Query(buildSQL(tblFlow), cutoff, until)
	if err != nil {
		t.mu.RUnlock()
		return nil, fmt.Errorf("query hot flow_samples timeline: %w", err)
	}
	hot, err := scan(hotRows)
	t.mu.RUnlock()
	if err != nil {
		return nil, fmt.Errorf("scan hot flow_samples timeline: %w", err)
	}

	return mergeSimpleAgg(sealed, hot), nil
}

type portDPIRow struct {
	Key   tsPortKey
	Svc   string
	Bytes uint64
}

func (t *tsStore) topDPIServiceForPorts(cutoff, until int64, keys []tsPortKey) (map[tsPortKey]string, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	...
	for i, k := range keys {
		...
		fmt.Fprintf(&inClause, "(%d, %d)", k.Proto, k.Port)
	}
	where := fmt.Sprintf("ts >= ? AND ts <= ? AND (proto, port) IN (%s)", inClause.String())
	buildSQL := func(source string) string {
		return fmt.Sprintf(`SELECT proto, port, COALESCE(dpi_service, ''), SUM(bytes) FROM %s WHERE %s GROUP BY proto, port, COALESCE(dpi_service, '')`, source, where)
	}
	scan := func(rows *sql.Rows) ([]portDPIRow, error) {
		defer rows.Close()
		var out []portDPIRow
		for rows.Next() {
			var proto, port, bytes int64
			var svc string
			if err := rows.Scan(&proto, &port, &svc, &bytes); err != nil {
				return nil, err
			}
			out = append(out, portDPIRow{Key: tsPortKey{Proto: uint8(proto), Port: uint16(port)}, Svc: svc, Bytes: uint64(bytes)})
		}
		return out, rows.Err()
	}

	sealed, err := querySealedAggSource(t, tblPort, portSealedSource, buildSQL, cutoff, until, scan)
	if err != nil {
		return nil, fmt.Errorf("query sealed port_samples dpi breakdown: %w", err)
	}

	t.mu.RLock()
	hotRowsSQL, err := t.hotDB.Query(buildSQL(tblPort), cutoff, until)
	if err != nil {
		t.mu.RUnlock()
		return nil, fmt.Errorf("query hot port_samples dpi breakdown: %w", err)
	}
	hot, err := scan(hotRowsSQL)
	t.mu.RUnlock()
	if err != nil {
		return nil, fmt.Errorf("scan hot port_samples dpi breakdown: %w", err)
	}

	totals := map[tsPortKey]map[string]uint64{}
	for _, r := range append(sealed, hot...) {
		if totals[r.Key] == nil {
			totals[r.Key] = map[string]uint64{}
		}
		totals[r.Key][r.Svc] += r.Bytes
	}

	winners := map[tsPortKey]string{}
	for key, bySvc := range totals {
		var bestSvc string
		var bestBytes uint64
		for svc, b := range bySvc {
			if b > bestBytes {
				bestBytes = b
				bestSvc = svc
			}
...
func (t *tsStore) queryPorts(cutoff, until int64) ([]portAggRow, error) {
	buildSQL := func(source string) string {
		return fmt.Sprintf(`SELECT proto, port, SUM(packets), SUM(bytes), MAX(dpi_service) FROM %s WHERE ts >= ? AND ts <= ? GROUP BY proto, port`, source)
	}
	scan := func(rows *sql.Rows) ([]portAggRow, error) {
		defer rows.Close()
		var out []portAggRow
		for rows.Next() {
			var proto, port, packets, bytes int64
			var dpiService sql.NullString
			if err := rows.Scan(&proto, &port, &packets, &bytes, &dpiService); err != nil {
				return nil, err
			}
			out = append(out, portAggRow{Key: tsPortKey{Proto: uint8(proto), Port: uint16(port)}, Packets: uint64(packets), Bytes: uint64(bytes), DPIService: dpiService.String})
		}
		return out, rows.Err()
	}
	...
	return mergePortAgg(sealed, hot), nil
}
...
func (t *tsStore) queryFlowsLimited(cutoff, until int64, ipFilter *uint32, q string, dpiOnly bool, limit, offset int, wantTotal bool) ([]flowAggRow, int, error) {
	...
	if q != "" {
		...
		for p, name := range ianaProtoNames {
			if strings.Contains(name, needle) {
				protoMatches = append(protoMatches, strconv.Itoa(int(p)))
			}
		}
		clause := `(` + ipOctetExpr("src_ip") + ` LIKE ?
			OR ` + ipOctetExpr("dst_ip") + ` LIKE ?
			OR CAST(src_port AS VARCHAR) LIKE ?
			OR CAST(dst_port AS VARCHAR) LIKE ?
			OR LOWER(COALESCE(domain, '')) LIKE ?`
		whereArgs = append(whereArgs, like, like, like, like, like)
		if len(protoMatches) > 0 {
			clause += fmt.Sprintf(" OR proto IN (%s)", strings.Join(protoMatches, ","))
		}
		clause += ")"
		where += " AND " + clause
	}
	if dpiOnly {
		where += " AND dpi_service IS NOT NULL AND dpi_service != ''"
	}
	baseArgs := func() []any {
		return append([]any{cutoff, until}, whereArgs...)
	}
	...
	if len(files) == 0 {
		...
		// query instead.
		rowSQL := fmt.Sprintf(`SELECT src_ip, src_port, dst_ip, dst_port, proto, SUM(packets), SUM(bytes), MAX(domain), MAX(dpi_service), MAX(svc_port)
		...
		defer t.mu.RUnlock()
		queryStart := time.Now()
		rows, err := t.hotDB.Query(rowSQL, append(baseArgs(), limit, offset)...)
		if err != nil {
		...
		total := 0
		if wantTotal {
			total, err = t.cachedTotal(flowTotalCacheKey(cutoff, until, ipFilter, q, dpiOnly), func() (int, error) {
				var n int
				if err := t.hotDB.QueryRow(countSQL(tblFlow), baseArgs()...).Scan(&n); err != nil {
	...
	buildSQL := func(source string) string {
		return fmt.Sprintf(`SELECT src_ip, src_port, dst_ip, dst_port, proto, SUM(packets), SUM(bytes), MAX(domain), MAX(dpi_service), MAX(svc_port)
			FROM %s WHERE %s GROUP BY src_ip, src_port, dst_ip, dst_port, proto
	...
	if len(liveFiles) > 0 {
		...
		rowsSQL, err := scratch.Query(buildSQL(liveSource), append(baseArgs(), fetchK)...)
		if err != nil {
			scratch.Close()
	...
	t.mu.RLock()
	hotRowsSQL, err := t.hotDB.Query(buildSQL(tblFlow), append(baseArgs(), fetchK)...)
	if err != nil {
		t.mu.RUnlock()
		return nil, 0, fmt.Errorf("query hot flow_samples: %w", err)
	}
	...
	if wantTotal {
		total, err = t.cachedTotal(flowTotalCacheKey(cutoff, until, ipFilter, q, dpiOnly), func() (int, error) {
			...
			if len(liveFiles) > 0 {
				...
				liveSource := flowSealedSource(liveFiles)
				var liveTotal int
				if err := scratch2.QueryRow(countSQL(liveSource), baseArgs()...).Scan(&liveTotal); err != nil {
			...
			defer t.mu.RUnlock()
			var hotTotal int
			if err := t.hotDB.QueryRow(countSQL(tblFlow), baseArgs()...).Scan(&hotTotal); err != nil {
				return 0, fmt.Errorf("count hot flow_samples: %w", err)
			}
...
func (t *tsStore) queryPortsLimited(cutoff, until int64, dpiOnly bool, limit, offset int, wantTotal bool) ([]portAggRow, int, error) {
	if limit+offset > maxFetchK {
		return nil, 0, errFetchTooDeep
	}
	...
	countSQL := func(source string) string {
		return fmt.Sprintf(`SELECT COUNT(*) FROM (SELECT 1 FROM %s WHERE %s GROUP BY proto, port)`, source, where)
	}

	if len(files) == 0 {
		t.mu.RLock()
		defer t.mu.RUnlock()
		rows, err := t.hotDB.Query(fmt.Sprintf(`SELECT proto, port, SUM(packets), SUM(bytes), MAX(dpi_service) FROM port_samples
			WHERE %s GROUP BY proto, port ORDER BY SUM(bytes) DESC LIMIT ? OFFSET ?`, where), cutoff, until, limit, offset)
		if err != nil {
	...
	buildSQL := func(source string) string {
		return fmt.Sprintf(`SELECT proto, port, SUM(packets), SUM(bytes), MAX(dpi_service) FROM %s WHERE %s GROUP BY proto, port ORDER BY SUM(bytes) DESC LIMIT ?`, source, where)
	}
	scan := func(rows *sql.Rows) ([]portAggRow, error) {
		defer rows.Close()
		var out []portAggRow
	...
	sealedSource := portSealedSource(files)
	sealedRowsSQL, err := scratch.Query(buildSQL(sealedSource), cutoff, until, fetchK)
	if err != nil {
		scratch.Close()
	...
	if wantTotal {
		...
		hotPortRows, err := t.hotDB.Query(fmt.Sprintf(`SELECT DISTINCT proto, port FROM port_samples WHERE %s`, where), cutoff, until)
		if err != nil {
			t.mu.RUnlock()
		...
		if len(hotKeys) > 0 {
			...
			excl.WriteByte(')')
			sealedExclusive = fmt.Sprintf(`SELECT COUNT(*) FROM (SELECT 1 FROM %s WHERE %s%s GROUP BY proto, port)`, sealedSource, where, excl.String())
		}
		var sealedOnlyTotal int
		if err := scratch2.QueryRow(sealedExclusive, cutoff, until).Scan(&sealedOnlyTotal); err != nil {
			scratch2.Close()
	...
	merged := mergePortAgg(sealedRows, hotRows)
	sort.Slice(merged, func(i, j int) bool { return merged[i].Bytes > merged[j].Bytes })
	return limitOffsetSlice(merged, limit, offset), total, nil
}
