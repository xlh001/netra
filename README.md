...
生产环境实测：约 1Gbps / 30万 pps 的持续流量下，96 核机器上 netra 进程仅占用约 1.6 个核（1.7%）。按这个实测占用比例折算到不同核数：
...
<sub>以上数据来自单台约 1Gbps / 30万 pps 的生产环境实测，更大流量场景暂时还没有实测数据。如果你的部署流量规模明显更大，欢迎在 [Issues](https://github.com/xxddpac/netra/issues) 里反馈实际观察到的 CPU 占用情况。</sub>
